...
For the sake of repetition, the requirements that need to be met are:
...
\begin{enumerate}
	\item Enable the computer, utilizing Matlab, to transmit a 10000 packets to the micro processor.
...
\end{table}
The test revealed that more packages are correctly received and decoded than control loops are executed. This suggests, as the data is sent periodically, that 
...
\fxnote{put figure for the test}
...
\begin{figure}[H]
    \includegraphics[scale=.7]{figures/pitchRefAcceptAllAngles.pdf}
    \centering			
