...
\subsection{Battery}
The battery available for the prototype, is a Zippy Flightmax battery. It weighs \SI{141}{g}, has a capacity of \SI{1500}{mAh}, a voltage of \SI{11.1}{V} and a discharge current of \SI{20}{A}.\fxnote{source!}
...
In order to use these modules, the packet has to be sent through the serial port when using the computer or through the USART module when using the microcontroller\fxnote{This is reffering to the explorer module used by the computer and the shield for the arduino I assume - we should write more clearly - present both pieces of hardware with an image (side by side) - also as far as I know both of these serves only as a direct wire connection between serial interface and Xbee module, we should be more specific - at time of writing I do not have access to the hardware, will look into it later - Niels.}.
