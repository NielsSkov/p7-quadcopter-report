...
The checksums error handling is not bulletproof, as an error can erase another error. An example could be if a bit in part 1 is changed from false to true and another bit in the same position in part 2 is change from true to false. Then the checksum is the same and the error is not detected. 

The protocol for the transport layer which is utilized on top of the xbee has been created.