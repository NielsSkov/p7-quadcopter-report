...
\begin{figure}[H]
	...
	\caption{System response of the attitude controller, with initial conditions of $0.2$ rad for the pitch, $-0.3$ rad for the roll and $-0.2$ rad for the yaw. The weighted matrices, $\vec{Q_{final}}$ and $\vec{R_{final}}$, and the observer gain, $\vec{L_{obs}}$, are utilized for the final design and can be found in \autoref{app:matricesSS}. \fxnote{change title and put final}}
	\label{fig:ssFinalEq}
\end{figure}
%
