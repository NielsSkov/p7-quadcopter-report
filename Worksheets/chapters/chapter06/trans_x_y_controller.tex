...
The step response for the designed controller with a negative gain of 0.907 can be seen in \autoref{fig:step_x}
\begin{figure}[H]
	\centering
...
\\
\\
Before it can be implemented and tested on the quadcopter, it needs to be discretized. As it is a P-controller, it simply means to encounter the sampling rate. The discretized controller is presented along with the continious controller in \fxnote{make graph}.
\\ \\
