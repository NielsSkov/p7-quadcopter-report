...
The bandwidth of the inner angular control is obtained by making a Bode plot approximation from the pitch angle reference to the real pitch angle in the quadcopter. The obtained Bode diagram is shown in \autoref{}. It can be seen that the frequency of the inner loop is 5 rad/s and this defines the bandtwidth of the velocity control loop to be 1.67 rad/s.
%


\fxnote{INCLUDE BODE DIAGRAM}
%\begin{figure}[H]
...
%\end{figure} 
%
For the velocity control, proportional controller is considered sufficient as there is an integrator in the plant that accounts for possible steady state errors and output disturbances. Input disturbances are considered not to occur as those are related with the motors and the inner attitude controller handles them. In order to design the controller, the Bode diagram of the transfer function has been used. It is shown in \autoref{}.

The value for the proportional gain is chosen can be seen directly in \autoref{} as the distance between the 0 dB level and the gain curve at the desired frequency of 1.67 rad/s. The resulting value is \fxnote{FINAL GAIN VALUE} dB, which results in a gain of \fxnote{FINAL GAIN VALUE}.
...
The proportional gain is chosen so that the bandwidth of the position loop is 3 times lower than that of the velocity loop. This defines it to be 0.56 rad/s. To set this bandwidth, the Bode plot of the plant shown in \autoref{} is used. The distance between the gain curve and the 0 dB level at 0.56 rad/s sets the gain of the controller to be \fxnote{GAIN OF POSITION CONTROLLER} dB or  
%{}
%\begin{figure}[H]
