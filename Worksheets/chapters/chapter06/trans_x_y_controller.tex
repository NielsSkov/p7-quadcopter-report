...
It is noticeable that, for this design, the variable changed by the controller is the pitch angle, which means that there is an inner loop (the attitude control loop) that is able to produce the required angle in the system. This is not accounted in the transfer functions. In order to start the design, the closed loop bandwidth of the control loop should be three to five times lower.\fxnote{SOURCE 3 TO 5 TIMES} 
...
In order to avoid that behavior, a zero at $-0.05$ is added to the controller to attract the branches to the stable region and the gain is adjusted to reduce the oscillations. The final root locus can be seen in \autoref{fig:rl_xdot}.
%
\begin{figure}[H]
