...
It is noticeable that, for this design, the variable changed by the controller is the pitch angle, which means that there is an inner loop (the attitude control loop) that is able to produce the required angle in the system. This is not accounted in the transfer functions. In order to start the design, the closed loop bandwidth of the control loop should be 3 to 5 times lower.\fxnote{SOURCE 3 TO 5 TIMES} 
...
%\end{figure} 
%
The plant is a type 1 system since it contains one integrator and the steady state error is handled by the naturally occurring integrator within the plant. However, due to input disturbances a steady state error is present in the output of the closed loop. 
...
From \autoref{eq:dc_gain_integrator} it is revealed that the steady state error is eliminated. 

An integrator is therefore included in the control design. Since there already exists an integral term in the plant, the closed loop system becomes marginally unstable since the branches in the root locus are located in the imaginary axis.

In order to avoid that behavior, a zero is added to the controller to attract the branches to the stable region and the gain is adjusted to reduce the oscillations. The final root locus can be seen in \autoref{fig:rl_xdot}.
%
\begin{figure}[H]
	\includegraphics[scale=.5]{figures/rl_xdot}
	\centering			
	\captionof{figure}{Root locus including the plant and the controller for the $x_{\mathrm{I}}$ translational velocity.} 
	\label{fig:rl_xdot}
\end{figure}
%
The bode plot in \autoref{bodeVelocity} shows that the final bandwidth of the closed loop system is 0.7 rad s$^-1$.
%
\begin{figure}[H]
