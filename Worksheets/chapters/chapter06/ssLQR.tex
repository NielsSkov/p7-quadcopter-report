...
The state feedback gain, $\vec{F}_e$, can be calculated using the expression seen in \autoref{eq:optimalF}. \cite{OptimalControlChristoffer}
\begin{flalign} 
	\vec{F}_e &= -\vec{R}^{-1}\vec{B}^T\vec{P}
	\label{eq:optimalF}
\end{flalign}
\begin{where}
