...
The system of the quadcopter has been described by both a linear and a non-linear mathematical model, these can be found in \autoref{chap:Model}. The models are utilized when designing the control system. The controllers are designed to be able to fulfil the relevant functional requirements stated in \autoref{ch:functionalRequirements}.
...
The rotational speed calculator receives the total speed of each motor and the required sum needed to get the desired thrust. The rotational speed calculator's objective is to combine it's five inputs into the final control action applied to the system. Yielding a new inertial position.
