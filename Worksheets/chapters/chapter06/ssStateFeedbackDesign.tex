...
\begin{minipage}{\linewidth}
	\begin{minipage}{0.5\linewidth}
		\begin{figure}[H]
			\includegraphics[scale=.3]{figures/ControllerColorDiagram}
			\centering			
			\captionof{figure}{Control structure highlighting the state feedback and integral control \fxnote{color the new one}.}
...
\end{flalign} 
%
This last equation is included in the existing state space model, having the system states, $\vec{x}$, and taking \si{\vec{x}_{\mathrm{Int}}} as new states, yielding the expression shown in \autoref{xdotSSExtended} and \ref{ySSExtended} \cite{ssReference}.
%
\begin{flalign} 
