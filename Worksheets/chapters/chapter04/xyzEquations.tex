...
In this section, the translational behavior of the quadcopter is modeled. First, the dynamics are described \textbf{in the body frame after which it is translated to the inertial frame.}
From the figure in the previous section, \fxnote{insert figure reference}, it is possible to derive equations for the translational accelerations in the body frame using Newton's second law.
...
In this case the gravity force has effect only in the \si{z_I} direction and the four forces from the motors need to be transformed into the inertial system. This is done using the inverse of the previously calculated rotation matrix (\eqref{invRotMatrix}).
%
\begin{flalign}
