\chapter{Model}
To make the quadcopter have a steady flight it is convenient to derive a model of the system, so the physical behavior of the quadcopter is described. From this, a control system must be designed such that the desired flying characteristics are obtained.

First, an overview is presented. Then, the model is derived and lastly, it is linearized as this is necessary to be able to design a linear control system. Finally, the model and the linearized model are compared in simulations. This ensures that the linear approximation yields acceptable results.

\section{Model Overview}
The model is split into two sub models, one being an angular model and the other being a translational velocity model. These are later combined to fully simulate the response of the system.
\begin{figure}[H]
    \centering
